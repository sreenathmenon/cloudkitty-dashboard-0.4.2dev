...
def is_billing_admin(request):
    ...
    for role in roles:
        if role.id == BILLING_ADMIN_ROLE or role.name == BILLING_ADMIN_ROLE:
            return True
    return False
...
class RescopeTokenToProject(tables.LinkAction):
    ...
    def get_link_url(self, project):
        ...
        # back to this page
        dash_url = reverse("horizon:identity:xprojects:index")
...
class UpdateBillingLink(tables.LinkAction):
    name = "billing"
    verbose_name = _("Modify Billing")
    url = "horizon:identity:xprojects:update"
    classes = ("ajax-modal",)
    icon = "pencil"
    #policy_rules = (("identity", "identity:list_groups"),)
    ...
    def allowed(self, request, project):
        return is_billing_admin(request)
...
class CreateProject(tables.LinkAction):
    ...
    url = "horizon:identity:xprojects:create"
    classes = ("ajax-modal",)
    icon = "plus"
...
class AdjustCreditAction(tables.LinkAction):
    name = "adjust_credit"
    verbose_name = _("Adjust Credit")
    url = "horizon:identity:xprojects:adjust_credit"
    classes = ("ajax-modal",)
...
class TenantsTable(tables.DataTable):
    name = tables.Column('name', verbose_name=_('Name'),
                         link=("horizon:identity:xprojects:detail"),
    ...
    credit = tables.Column('billing_credit', verbose_name=_('Credit'))
    ...
    class Meta(object):
        ...
        row_actions = (UpdateMembersLink, UpdateGroupsLink, UpdateProject,
                       ...
                       RescopeTokenToProject, UpdateBillingLink, AdjustCreditAction)
