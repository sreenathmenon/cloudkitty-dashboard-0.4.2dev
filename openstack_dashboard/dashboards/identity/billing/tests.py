...
from openstack_dashboard.dashboards.identity.billing import workflows
...
class ProjectsViewTests(test.BaseAdminViewTests):
    ...
    def test_index(self):
        api.keystone.tenant_list(IsA(http.HttpRequest),
                                 ...
                                 paginate=True,
                                 marker=None) \
            .AndReturn([self.projects.list(), False])
        self.mox.ReplayAll()

        res = self.client.get(INDEX_URL)
        self.assertTemplateUsed(res, 'identity/billing/index.html')
    ...
    def test_index_with_domain_context(self):
        ...
        domain_projects = [tenant for tenant in self.projects.list()
                          if tenant.domain_id == domain.id]
        api.keystone.tenant_list(IsA(http.HttpRequest),
                                 domain=domain.id,
                                 paginate=True,
                                 marker=None) \
                    .AndReturn([domain_projects, False])
        self.mox.ReplayAll()

        res = self.client.get(INDEX_URL)
        self.assertTemplateUsed(res, 'identity/billing/index.html')
        self.assertItemsEqual(res.context['table'].data, domain_projects)
...
class CreateProjectWorkflowTests(test.BaseAdminViewTests):
    ...
    def test_add_project_get(self):
        ...
        url = reverse('horizon:identity:billing:create')
        ...
        self.assertQuerysetEqual(
            ...
            ['<CreateProjectInfo: createprojectinfoaction>',
             '<UpdateBillingMembers: update_members>',
             '<UpdateBillingGroups: update_group_members>',
    ...
    def test_add_project_get_with_neutron(self):
        ...
        res = self.client.get(reverse('horizon:identity:billing:create'))
...
class UpdateBillingWorkflowTests(test.BaseAdminViewTests):
    def _get_quota_info(self, quota):
        cinder_quota = self.cinder_quotas.first()
        neutron_quota = self.neutron_quotas.first()
        quota_data = {}
    ...
    def test_update_project_get(self):
        ...
        url = reverse('horizon:identity:billing:update',
                      args=[self.tenant.id])
        res = self.client.get(url)
        ...
        self.assertQuerysetEqual(
            workflow.steps,
            ['<UpdateBillingInfo: update_info>',
             '<UpdateBillingMembers: update_members>',
             '<UpdateBillingGroups: update_group_members>',
             '<UpdateBillingQuota: update_quotas>'])
...
class UsageViewTests(test.BaseAdminViewTests):
    ...
    def _test_usage_csv(self, nova_stu_enabled=True):
        ...
        project_id = self.projects.first().id
        csv_url = reverse('horizon:identity:billing:usage',
...
class DetailProjectViewTests(test.BaseAdminViewTests):
    ...
    def test_detail_view(self):
        ...
        self.assertTemplateUsed(res, 'identity/billing/detail.html')
...
class SeleniumTests(test.SeleniumAdminTestCase):
    ...
    def test_inline_editing_update(self):
        ...
        api.keystone.tenant_get(IgnoreArg(),
                                u'1',
                                admin=True) \
            .AndReturn(self.projects.list()[0])
        ...
        changed_tenant = copy.copy(self.projects.list()[0])
        ...
        td_element = self.selenium.find_element_by_xpath(
            ...
            "&table=projects&cell_name=name&obj_id=1']")
    ...
    def test_membership_list_loads_correctly(self):
        ...
        self.selenium.get("%s%s" %
                          (self.live_server_url,
                           reverse('horizon:identity:billing:create')))
